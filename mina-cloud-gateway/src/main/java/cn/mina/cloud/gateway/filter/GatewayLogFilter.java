...
/**
 * <p> 网关请求响应日志打印 </p>
 * order值要小于-1，因为只有小于-1（即比NettyWriteResponseFilter.WRITE_RESPONSE_FILTER_ORDER小），writeWith方法才会执行
 */
@Slf4j
public class GatewayLogFilter implements GlobalFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        TraceLog traceLog = TraceLog.builder()
                ...
                .requestTime(DateUtil.nowStr(DateUtil.DATE_TIME_MS_FORMAT))
                .build();
    ...
    public Mono<Void> writeLog(ServerWebExchange exchange, GatewayFilterChain chain, TraceLog traceLog) {
        ...
        return chain.filter(exchange.mutate().response(serverHttpResponseDecorator).build())
                .then(Mono.fromRunnable(() -> traceLog.logResponseStr()));
    }
    ...
    public Mono<Void> writeBodyLog(ServerWebExchange exchange, GatewayFilterChain chain, TraceLog traceLog) {
        ...
        }).doFinally(body -> traceLog.logRequestStr());
        ...
        return new ServerHttpResponseDecorator(response) {
            ...
            public Mono<Void> writeWith(Publisher<? extends DataBuffer> body) {
                if (body instanceof Flux) {
                    traceLog.setResponseTime(DateUtil.nowMsStr());
                    traceLog.setExecuteTime(DateUtil.diffMillisecond(
                            DateUtil.strToDate(traceLog.getRequestTime(), DateUtil.DATE_TIME_MS_FORMAT),
                            DateUtil.strToDate(traceLog.getResponseTime(), DateUtil.DATE_TIME_MS_FORMAT)
                    ));
